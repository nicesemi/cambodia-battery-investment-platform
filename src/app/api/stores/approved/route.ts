...
export async function GET(request: Request) {
  try {
    // 允许公开访问：首页需要展示门店数据（未登录用户可查看）
    let user = null
    try {
      user = await authenticateToken(request)
    } catch {}
    ...
    // 构建 store → investors 反向映射（本月业绩计算用）
    ...
    const stores = (data || []).map((s: any) => {
      ...
      return {
        ...
        investor_count: perf.investor_count || 0,

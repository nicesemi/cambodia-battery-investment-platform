        ...
        <div className="card">
          ...
          {selectedUnitIds.length > 0 && (
            <button onClick={handleSellToPlatform} disabled={selling}
              ...
              {selling ? '处理中...' : `出售 ${selectedUnitIds.length} 个单元给平台 — 合计 $${totalBuyback.toFixed(2)}`}
            </button>)}
          {sellMsg && (
                ...
                return (
                  <div key={selectedUnitIds[i]} className="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="p-3 space-y-1.5 text-sm">
                      <div className="flex justify-between"><span className="text-gray-500">购入价</span><span className="font-medium">${p.purchasePrice?.toFixed(2)}</span></div>
                      ...
                      <div className="flex justify-between"><span className="text-gray-500">残值</span><span className="font-medium">${p.residualValue?.toFixed(2)} ({p.residualRate}%)</span></div>
                      {p.penaltyRate > 0 && (
                        <div className="flex justify-between"><span className="text-red-500">罚金 ({p.penaltyRate}%)</span><span className="font-medium text-red-500">-${p.penaltyAmount?.toFixed(2)}</span></div>
                      )}
                      <div className="border-t pt-1.5 flex justify-between">
                        <span className="font-medium text-primary-900">回购价</span>
                        <span className="font-bold text-primary-700">${p.buybackPrice?.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                );

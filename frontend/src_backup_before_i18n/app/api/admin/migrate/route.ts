...
export async function POST(request: Request) {
  try {
    ...
    results.push(await execSQL(adminClient, `ALTER TABLE workers ADD COLUMN IF NOT EXISTS age INTEGER`))
    results.push(await execSQL(adminClient, `ALTER TABLE workers ADD COLUMN IF NOT EXISTS worker_code TEXT`))
    results.push(await execSQL(adminClient, `CREATE UNIQUE INDEX IF NOT EXISTS idx_workers_worker_code ON workers(worker_code)`))

    // 7. RLS
    for (const tbl of ['investor_store_bindings', 'store_performance', 'agent_applications', 'workers']) {

...
export default function Trade() {
  ...
  const handleSellToPlatform = async () => {
    ...
    try {
      ...
      setSellMsg(`成功！共获得 $${json.totalBuyback.toFixed(2)} (≈ ¥${(json.totalBuyback * 7.25).toFixed(2)})，新余额 $${json.newBalance.toFixed(2)} (≈ ¥${(json.newBalance * 7.25).toFixed(2)})`);
      setSelectedUnitIds([]);
      setPreviewMap({});
      loadMyUnits();
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="card">
            ...
            : <div className="space-y-2 max-h-96 overflow-y-auto">
                {myUnits.map(unit => (
                  <div key={unit.id} onClick={() => toggleUnitSelection(unit.id)}
                    ...
                    <div className="mt-1 text-xs text-gray-500">购入价: ${Number(unit.unit_price || 1000).toFixed(2)} (≈ ¥{(Number(unit.unit_price || 1000) * 7.25).toFixed(2)})</div>
                  </div>))}
              </div>}
          {selectedUnitIds.length > 0 && (
            <button onClick={handleSellToPlatform} disabled={selling}
              ...
              {selling ? '处理中...' : `出售 ${selectedUnitIds.length} 个单元给平台 — 合计 $${totalBuyback.toFixed(2)} (≈ ¥${(totalBuyback * 7.25).toFixed(2)})`}
            </button>)}
          {sellMsg && (
        ...
        <div className="card">
          ...
          {selectedPreviews.length === 0 && <div className="text-sm text-gray-400 text-center py-12">点击左侧电池单元查看回购价格</div>}
            ...
            <div className="space-y-3">
                ...
                return (
                  <div key={selectedUnitIds[i]} className="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="p-3 space-y-1.5 text-sm">
                      <div className="flex justify-between"><span className="text-gray-500">购入价</span><span className="font-medium">${p.purchasePrice?.toFixed(2)} (≈ ¥{(p.purchasePrice * 7.25).toFixed(2)})</span></div>
                      <div className="flex justify-between"><span className="text-gray-500">持有</span><span className="font-medium">{p.monthsHeld?.toFixed(1)} 月</span></div>
                      <div className="flex justify-between"><span className="text-gray-500">残值</span><span className="font-medium">${p.residualValue?.toFixed(2)} ({p.residualRate}%) (≈ ¥{(p.residualValue * 7.25).toFixed(2)})</span></div>
                      {p.penaltyRate > 0 && (
                        <div className="flex justify-between"><span className="text-red-500">罚金 ({p.penaltyRate}%)</span><span className="font-medium text-red-500">-${p.penaltyAmount?.toFixed(2)} (≈ ¥{(p.penaltyAmount * 7.25).toFixed(2)})</span></div>
                      )}
                      <div className="border-t pt-1.5 flex justify-between">
                        <span className="font-medium text-primary-900">回购价</span>
              ...
              <div className="bg-primary-50 rounded-lg p-4 border border-primary-200">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-primary-600">选中 {selectedPreviews.length} 个单元 · 合计回购</span>

  ...
  try {
    ...
    const [{ data: assets, error }, { data: types }] = await Promise.all([
      supabase.from('battery_assets')
        .select('id, asset_code, name, description, battery_type, battery_type_id, total_units, available_units, unit_price, unit_price_rmb, expected_roi, monthly_rent, location, status, created_at')
        .order('created_at', { ascending: false }),
      admin.from('battery_types').select('id, image_url, monthly_rent').eq('is_active', true)
    ])
    if (error) return serverError(error.message)

    // 构建 battery_type_id → thumbnail_url / monthly_rent 映射
    ...
    const formatted = (assets || []).map((a: any) => ({
      ...a,
      thumbnail_url: imageMap[a.battery_type_id] || null,
      monthly_rent: rentMap[a.battery_type_id] ?? null
    }))

...
export async function GET(request: Request) {
  try {
    ...
    const [{ data: assets, error }, { data: types }, { data: sites }] = await Promise.all([
      ...
      admin.from('operation_sites').select('country, city, country_i18n, city_i18n')
    ])
    ...
    for (const t of (types || [])) {
      if (t.id) {
        ...
        if (t.unit_price != null) priceMap[t.id] = t.unit_price
        // 年化收益率 = 月租金 × 12 × 70%（净收益比例） / 单价 × 100
        if (t.monthly_rent != null && t.unit_price != null && t.unit_price > 0) {
          returnMap[t.id] = parseFloat(((t.monthly_rent * 12 * 0.7) / t.unit_price * 100).toFixed(2))
        }
        typeNameMap[t.id] = { name: t.name, name_i18n: t.name_i18n }
      }
    }

    // 构建 "country·city" → location_i18n 映射
    const locationI18nMap: Record<string, any> = {}
    for (const s of (sites || [])) {
      const key = `${s.country}·${s.city}`
      if (!locationI18nMap[key] && s.country_i18n && s.city_i18n) {
        const locI18n: Record<string, string> = {}
        for (const lang of SUPPORTED_LOCALES) {
          const countryName = resolveI18n(s.country_i18n, lang, '')
          const cityName = resolveI18n(s.city_i18n, lang, '')
          if (countryName || cityName) {
            locI18n[lang] = [countryName, cityName].filter(Boolean).join('·')
          }
        }
        if (Object.keys(locI18n).length > 0) {
          locationI18nMap[key] = locI18n
    ...
    const formatted = (assets || []).map((a: any) => {
      ...
      return {
        ...
        // 单价（USD）：battery_types.unit_price > asset
        unit_price: priceMap[a.battery_type_id] ?? a.unit_price ?? null,
        // 年化收益率：battery_types 计算 > battery_assets.expected_roi
        annualized_return: returnMap[a.battery_type_id] ?? a.expected_roi ?? null,
        // 多语言解析
        ...
        resolved_battery_type: btTypeName
          ...
          : (a.battery_type || ''),
        location_i18n: locationI18nMap[a.location] || null,
        resolved_location: resolveI18n(locationI18nMap[a.location], locale, a.location || ''),
      }
    })

...
export async function PUT(request: Request, { params }: { params: { typeId: string } }) {
  try {
    ...
    const { name, voltage, capacity, chemistry, description, is_active, sort_order, image_url, scenario, dimensions, net_weight, power_kwh, unit_price, monthly_rent, annualized_return } = body
    ...
    const { data: type, error } = await supabase.from('battery_types')
      .update({
        ...
        annualized_return: annualized_return !== undefined ? (annualized_return !== '' ? parseFloat(annualized_return) : null) : undefined,

...
export async function POST(request: Request) {
  try {
    ...
    const { data: user, error } = await supabase.from('users')
      .select('id, email, username, password_hash, full_name, role, is_active, agent_type')
    ...
    return ok({ message: 'Login successful', token, user: { id: user.id, email: user.email, username: user.username, fullName: user.full_name, role: user.role, agentType: user.agent_type || null } })
  } catch (e: any) {
